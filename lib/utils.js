...
module.exports = {
    ...
    isEmpty: function(a) {
        return typeof(a)=='undefined' || !a || a == '0' || a === 0
    },
    ...
    getQueryString: function (urlString, key) { // 获取url链接的参数值
        var strArray = urlString.replace(/^\?/,'').split('?'); // 按照?拆分
        if (strArray.length < 2) {
          return ''; // 参数不存在
        }
        var paramsArray = strArray[1].replace(/^\?/,'').split('&'); // 获取?后面的参数串并拆分数组
        var paramsObj = {};
        for(var i = 0, iLen = paramsArray.length; i < iLen; i++){
    ...
    getDateStr: function (addDayCount) { // 获取日期：addDayCount=1表示明天；=2表示后天
      var dd = new Date();
